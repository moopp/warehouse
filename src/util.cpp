...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\YDChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\YDChain
    // Mac: ~/Library/Application Support/YDChain
    // Unix: ~/.YDChain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YDChain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "YDChain.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty YDChain.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
		fputs("rpcuser=user \nrpcpassword=123456 \nserver=1 \nlisten=1 \nrpcallowip=127.0.0.1\nrpcport=27909", configFile);
        if (configFile != NULL)
            fclose(configFile);
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "YDChaind.pid"));
